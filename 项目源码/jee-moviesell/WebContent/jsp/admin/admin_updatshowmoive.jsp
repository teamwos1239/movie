<%@page import="com.chinasoft.moviesell.domain.Showmoive"%>
		...
		<div id="" class="main_main">
			...
			<div id="content_right" class="content_right">
				...
				<div id="main_tit2" class="main_tit2">
				  <img src="/jee-moviesell/pic/admin/arrow.png" width="14" height="17">修改电影放映信息
				</div>				
				<p>请输入重新电影的详细放映信息</p>
				...
				<div id="main_main" class="main_main">
				 ...
				 <div id="addmovie" class="addmovie">
				 	<form id="loginform" name="loginform" action="/jee-moviesell/UpdateShowmoiveServlet" method="post" onsubmit="return checkSub();">
				 		<input type="hidden" value="${requestScope.findshowmoive.sno }" name="sno"/>
			            <table id="addmovie_tab">
			                <tr>
			                    <td class="tab_left">电影院：</td>
			                    <td>
			                    <select id=" " class=" " name="tno">
										<c:forEach items="${requestScope.findlstTheatres }" var="theatres" varStatus="i">
											<c:if test="${requestScope.findshowmoive.tno==theatres.tno }">
												<option value="${theatres.tno}" selected="selected">${theatres.tname }</option>
											</c:if>
											<c:if test="${requestScope.findshowmoive.tno!=theatres.tno }">
			                ...
			                <tr>
			                    ...
			                    <select id=" " class=" " name="mno">
										<c:forEach items="${requestScope.findlstMovie }" var="movie" varStatus="i">
											<c:if test="${requestScope.findshowmoive.mno==movie.mno }">
												<option value="${movie.mno}" selected="selected">${movie.mname }</option>
											</c:if>
											<c:if test="${requestScope.findshowmoive.mno!=movie.mno }">
												<option value="${movie.mno}">${movie.mname }</option>
											</c:if>
					 					</c:forEach>
								</select>
			                    ...
			                    <td>
			                     <%!String strTime; %>
								  <%
									...
									if(request.getAttribute("findshowmoive")!=null){
										Showmoive showmoive=(Showmoive)request.getAttribute("findshowmoive");
										strTime=dateFormat.format(showmoive.getStime());
										
									}
								  %>
			                    	<input value="<%=strTime %>" type="text" id="stime" name="stimeYuan" class="Wdate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})" readonly/>
								</td>
			                    <td><span id="" class="" style="font-size:12px;"></span></td>
			                </tr>
			                <tr>
			                    <td class="tab_left">厅：</td>
			                    <td><input value="${requestScope.findshowmoive.shallno }" type="text" id="" name="shallno" /></td>
			                    <td></td>
			                </tr>
			                <tr>
			                  <td class="tab_left">电影票价格：</td>
			                  <td><input value="${requestScope.findshowmoive.sprice }" type="text" class="" name="sprice" id="" /></td>
			                  <td></td>
		                 	</tr> 
			                <tr>
			                  <td class="tab_left">语言：</td>
			                  <td><input value="${requestScope.findshowmoive.slanguage }" type="text" class="" name="slanguage" id="" /></td>
			                  <td></td>
		                  	</tr>
			                 ...
			                 <div id="seatSelect" class="">
				                 	...
				                 	<div id="seatSe_main" class="seatSe_main">
					                 	...
					                 	<c:forEach items="${requestScope.findlstSeat}" var="seats" varStatus="i">
										<!--  -->
												...
												<c:forEach items="${seats}" var="seatFlag" varStatus="j">
												 	<div class="seat_item">
														<c:if test="${seatFlag==0}">
															<div class="seat_img3" id="seat_${i.count }_${j.count}" onclick="seatclick(this.id,'value_${i.count}_${j.count}',${i.count},${j.count});" >
															</div>${j.count}
															<input type="hidden" value="0" name="value_${i.count}" id="value_${i.count}_${j.count}"/>
														</c:if>
														<c:if test="${seatFlag==1}">
															<div class="seat_img0">
															</div>${j.count}
															<input type="hidden" value="1" name="value_${i.count}" id="value_${i.count}_${j.count}"/>
														</c:if>
					 ...
					 <!--提示  -->
						...
						<c:if test="${!empty requestScope.updateshowmoiveflag}">
							<div class="addcityflag" id="addcityflag">
								<c:if test="${requestScope.updateshowmoiveflag==true}">
									<script>
										alert('电影修改成功~');
									</script>
								</c:if> 
								<c:if test="${requestScope.updateshowmoiveflag==false}">
									<script>
										alert('电影修改失败~');
									</script>
								</c:if> 
