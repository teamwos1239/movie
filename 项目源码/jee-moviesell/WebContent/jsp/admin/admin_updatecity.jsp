			...
			<div id="" class="main_main">
				...
				<div id="content_right" class="content_right">
					...
					<div id="main_tit2" class="main_tit2">
						<img src="/jee-moviesell/pic/admin/arrow.png" width="14" height="17">修改城市信息
					</div>
					<p>请重新输入城市的详细信息</p>
					...
					<div id="main_main" class="main_main">
						...
						<div id="addmovie" class="addmovie">
							<form id="loginform" name="loginform" action="/jee-moviesell/UpdateCitiesServlet" method="post">
								<input type="hidden" id="" name="cno" value="${requestScope.findcity.cno }" />
								<table id="addmovie_tab">
									...
									<tr>
										...
										<td><input type="text" class="" name="cname" id="" value="${requestScope.findcity.cname }" /></td>
									...
									<tr>
										...
										<td><select name="cprovince" id="" class="">
												<c:forEach items="${citiesarr }" var="citys">
													<c:if test="${citys!=requestScope.findcity.cprovince }">
														<option value="${citys }">${citys }</option>
													</c:if>
													<c:if test="${citys==requestScope.findcity.cprovince }">
														<option value="${citys }" selected="selected">${citys }</option>
													</c:if>
								...
								<div
									...
									<input type="submit" value="确定 修改" id="enrollsubmit"
						...
						<c:if test="${!empty requestScope.updatecityflag}">
							<div class="addcityflag" id="addcityflag">
								<c:if test="${requestScope.updatecityflag==true}">
									<script>
										alert('城市修改成功~');
									</script>
								</c:if> 
								<c:if test="${requestScope.updatecityflag==false}">
										<script>
										alert('城市修改失败~');
									</script>
								</c:if> 
