...
function deleteCheck(pagePath,uno){
	
	if(confirm('确定删除吗？')){
		location='/jee-moviesell/DeleteUserServlet?uno='+uno+"&&pagePath="+pagePath;
	}
...
<c:if test="${requestScope.userPage==null }">
	...
	<script type="text/javascript">
			location = "/jee-moviesell/ShowUsersServlet?currentPage=" + 1;
	</script>
</c:if>
...
<script type="text/javascript">
	function gotoPage(page) {
		var url = "/jee-moviesell/ShowUsersServlet?currentPage=" + page;
		location = url;
		...
		<div id="" class="main_main">
			<div id="" class="main_left">
				...
				<div class="admin_menu" id="admin_menu">
						<div id="admin_on" class="admin_tit" onClick="adminmenuOnclick('dd1');"
							style="">用户管理</div>
						<ul id="dd1" class="admin_con" style="">
							<li><a href="/jee-moviesell/jsp/admin/admin.jsp" class="o_on">查看所有用户</a></li>
			...
			<div id="content_right" class="content_right">
			  ...
			  <img src="/jee-moviesell/pic/admin/arrow.png" width="14" height="17">查看所有用户 </div>				
				...
				<div id="main_main" class="main_main">
					<table >
					  ...
					  <%
					   ...
					   <c:if test="${requestScope.userPage.list!=null}">
						  <c:forEach items="${requestScope.userPage.list}" var="user"  varStatus="i">
							   <tr class="tab_main">
						    <td>${i.count }</td>
						    <td>${user.uno }</td>
						    <td>${user.uemail }</td>
						    <td>${user.uname }</td>
						    <td>${user.ulevel }</td>
						    <td><button onclick="deleteCheck('${pagePath}',${user.uno});">删除</button></td>
						  </tr>
						  </c:forEach>
					  </c:if>
					  <c:if test="${empty requestScope.userPage.list}">
							  <tr class="tab_main">
					...
					<div class="page_to">
						...
						onchange="javascript:gotoPage(this.value);">
							...
							<c:forEach var="i" begin="1"
								end="${requestScope.userPage.totalPage }">
								<c:if test="${i!=requestScope.userPage.currentPage}">
					...
					<c:if test="${!empty requestScope.deluserflag}">
							<div class="addcityflag" id="addcityflag">
						
							<c:if test="${requestScope.deluserflag==true}">
								<script>
									alert('用户删除成功~');
								</script>
							</c:if> 
							<c:if test="${requestScope.deluserflag==false}">
								<script>
									alert('用户删除失败~');
								</script>
							</c:if> 
