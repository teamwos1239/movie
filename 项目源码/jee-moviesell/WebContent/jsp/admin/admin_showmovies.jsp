...
function deleteCheck(mno){
	
	if(confirm('确定删除吗？')){
		location='/jee-moviesell/DeleteMoviesServlet?mno='+mno;
	}
...
<c:if test="${requestScope.moviesPage==null }">

	<script type="text/javascript">
			location = "/jee-moviesell/ShowMoviesServlet?currentPage=" + 1;
	</script>
</c:if>
...
<script type="text/javascript">
	function gotoPage(page) {
		var url = "/jee-moviesell/ShowMoviesServlet?currentPage=" + page;
		location = url;
	...
	<div id="" class="content_main">
		...
		<div id="" class="main_main">
			<div id="" class="main_left">
				...
				<div class="admin_menu" id="admin_menu">
						...
						<ul id="dd3" class="admin_con" style="">
							<li><a href="/jee-moviesell/jsp/admin/admin_showmovies.jsp" class="o_on">查看所有电影</a></li>
			...
			<div id="content_right" class="content_right">
			  ...
			  <img src="/jee-moviesell/pic/admin/arrow.png" width="14" height="17">查看所有电影 </div>				
				...
				<div id="main_main" class="main_main">
					<table >
					  <tr class="tab_tit">
				       ...
				       <c:if test="${requestScope.moviesPage.list!=null}">
						  <c:forEach items="${requestScope.moviesPage.list}" var="movie"  varStatus="i">
							   <tr class="tab_main">
						    <td>${i.count }</td>
						    <td>${movie.mno}</td>
						    <td>${movie.mname }</td>
						    <td>${movie.mtype }</td>
						    <td>${movie.mstarttime }</td>
						    <td>${movie.mcountry }</td>
						    <td>${movie.mversion }</td>
						    <td><button onclick="javascript:location='/jee-moviesell/FindByIdMoviesServlet?mno=${movie.mno}';">修改</button><button onclick="deleteCheck(${movie.mno});">删除</button></td>
						  </tr>
						  </c:forEach>
					  </c:if>
					  <c:if test="${empty requestScope.moviesPage.list}">
							  <tr class="tab_main">
					...
					<div class="page_to">
						...
						onchange="javascript:gotoPage(this.value);">
							...
							<c:forEach var="i" begin="1"
								end="${requestScope.moviesPage.totalPage }">
								<c:if test="${i!=requestScope.moviesPage.currentPage}">
					...
					<c:if test="${!empty requestScope.delmoviesflag}">
							<div class="addcityflag" id="addcityflag">
						
							<c:if test="${requestScope.delmoviesflag==true}">
								<script>								
									alert('电影删除成功~');
								</script>
							</c:if> 
							<c:if test="${requestScope.delmoviesflag==false}">
								<script>
									alert('电影删除失败~');
								</script>
							</c:if> 
