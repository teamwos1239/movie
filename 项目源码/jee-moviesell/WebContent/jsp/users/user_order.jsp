...
<input id="unoHidden" value="${sessionScope.logineduser.uno }" type="hidden"/>

<c:if test="${requestScope.usreOrderPage==null }">

	<script type="text/javascript">
			location = "/jee-moviesell/FindUserSeeOrderServlet?currentPage=" + 1;
	</script>
</c:if>
...
<script type="text/javascript">
	function gotoPage(page) {
		var url = "/jee-moviesell/FindUserSeeOrderServlet?currentPage=" + page;
		location = url;
...
<div id="content" class="content" >
	...
	<div id="content_nav" class="content_nav" >
		<span id="" class="nav_off" >
			您当前的位置：<a href="/jee-moivesell/jsp/index.jsp">首页</a>&#62;<a href="/jee-moivesell/users/users.jsp">用户中心首页</a>&#62;
		</span>
		...
		<div id="" class="main_main">
			...
			<div id="" class="main_right">
				...
				<div id="" class="user_set main_tit">
			 		 <img src="/jee-moviesell/pic/user/user_order.png" width="42" height="45">我的订单
		 	  </div>		
			 		 
				<div id="" class="user_set_nav">
					<dl id="user_set_dl">
						<dd id="set_on" class="set_on"><a href="#">购票订单</a></dd>
						<dd id="" class=""><a href="#">购卡订单</a></dd>
						<dd id="" class=""><a href="#">储蓄卡充值</a></dd>
						<dd id="" class=""><a href="#">积分换礼</a></dd>
						<dd id="" class=""><a href="#">见面会订单</a></dd>
					</dl>				
				</div>			 		 
			 		 	<br>
			 		 	<br>
				...
				<div id="" class="main_main">				
					...
					<div id="" class="orders_main">				
					 	<table  >
						  <tr class="orders_tab_tit">
						    <td>下单时间</td>
						    <td>电影名称</td>
						    <td>影院信息</td>
						    <td>支付方式</td>
						 ...
						 <c:if test="${requestScope.usreOrderPage.list!=null}">
						  <c:forEach items="${requestScope.usreOrderPage.list}" var="orderItem"  varStatus="i">							 
							  <tr class="orders_tab_td">
							    <td>${orderItem.oOrder.otime}</td>
							    <td>${orderItem.oMovie.mname }</td>
							    <td><a href="javascript:location='/jee-moviesell/HelperFindTheatreServlet?tno=${orderItem.oTheatre.tno }';" target="_blank">${orderItem.oTheatre.tname }</a></td>
							    <td>${orderItem.oOrder.opayway }</td>
							    <td>${orderItem.oOrder.opaymoney }元</td>
							    <td>${orderItem.oOrder.oconvertcode }</td>
							    <td>
							    	<button class="" value="" onClick="javascript:location='/jee-moviesell/FindByIdOrderServlet?ono=${orderItem.oOrder.ono }';">详情</button>
							    </td>
							  </tr>
						  </c:forEach> 
						  </c:if>
					 	 <c:if test="${empty requestScope.usreOrderPage.list}">
							  <tr class="tab_main">
							...
							<div class="page_to" style="text-align: right;padding: 20px;">
								...
								onchange="javascript:gotoPage(this.value);">
									...
									<c:forEach var="i" begin="1"
										end="${requestScope.usreOrderPage.totalPage }">
										<c:if test="${i!=requestScope.usreOrderPage.currentPage}">
